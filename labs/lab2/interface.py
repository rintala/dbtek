...
# here columns becomes the string that you type at prompt for Choose columns.
    ...
    def remove(self):
        ...
        print("YOU HAVE CHOOSEN 'REMOVE'")
        table = raw_input("Choose table: ")
        try:
            #get our columns from selected table
            query = "SELECT column_name FROM (SELECT table_name, column_name, is_nullable, data_type FROM INFORMATION_SCHEMA.COLUMNS) AS t1 WHERE table_name='%s'" % (table)
        except (NameError,ValueError, TypeError,SyntaxError):
            print "  Bad input."
            return

        print("SUCCESSFULLY RETRIEVED COLS: ", query)
        ...
        for colName in cursor:
            qtoExecute = qtoExecute + ("%s = " % colName[0]) + raw_input("Input for %s: " % colName[0]) + " AND "

        
        qtoExecute= qtoExecute.strip(" AND ")
        print(qtoExecute)
        qtoExeComplete = "DELETE FROM %s" % table + " WHERE " + qtoExecute + ";"
        print qtoExeComplete
        
        self.cur.execute(qtoExeComplete)
    ...
    def insert(self):
        ...
        for colName in cursor:
            qtoExecute = qtoExecute + raw_input("Input for %s: " % colName[0]) + ", "

            print(qtoExecute)
        qtoExecute= qtoExecute.strip(", ")
        print(qtoExecute)

        try:
            qtoExeComplete = "INSERT INTO %s VALUES(%s);" % (table, qtoExecute)
        except (NameError,ValueError, TypeError,SyntaxError):
            print "  Bad input."
            return
